...
indexagesum %>% display_data(disp=.disp,)
```
...
patscohort %>% pivot_longer(-patid,names_to="cohort",values_to="cc") %>%
  ...
  dplyr::mutate(total=case+cont) %>% display_data(disp=.disp,)
```
...
if(nrow(res)>0) res %>%
  ...
  display_data(disp=.disp,caption="SLE patients")
...
if(nrow(res)>0) res %>%
  select(term,snomedctconceptid,case,cont,ratio,padj) %>%
  display_data(disp=.disp,caption="IBD patients")
res <- run_test(dbf,cohort=coh,symptomtab="symptomcodes",groupat="snomedctconceptid")
if(nrow(res)>0) res %>% 
  select(term,snomedctconceptid,case,cont,ratio,padj) %>%
  display_data(disp=.disp,caption="IBD patients")
...
if(nrow(res)>0) res %>%
  select(term,snomedctconceptid,case,cont,ratio,padj) %>%
  display_data(disp=.disp,caption="Behcet's patients")
res <- run_test(dbf,cohort=coh,symptomtab="symptomcodes",groupat="snomedctconceptid")
if(nrow(res)>0) res %>% 
  select(term,snomedctconceptid,case,cont,ratio,padj) %>%
  display_data(disp=.disp,caption="Behcet's patients")
