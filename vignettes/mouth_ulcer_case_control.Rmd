...
```{r}
res <- run_test(dbf,cohort='sle',symptomtab="symptomcodes",cortype="bon")
if(nrow(res)>0) res %>% kable(caption="SLE patients")
```

```{r}
res <- run_test(dbf,cohort='ibd',symptomtab="symptomcodes")
if(nrow(res)>0) res %>% kable(caption="IBD patients")
```

```{r}
res <- run_test(dbf,cohort='behcets',symptomtab="symptomcodes") 
