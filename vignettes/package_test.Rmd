...
dbif <- file.path(pdir,"testaurumdata.sqlite")
dir(ddir)
dir(rdir)
```
...
load_patients(pddir=pddir,dbf=dbif,ow=T,db=F)
```
...
load_obs(pddir=pddir,dbf=dbif,ow=T,db=F)
```

## Load CPRD references

```{r}
load_cprdfiles(pddir=rdir,dbf=dbif)
```
