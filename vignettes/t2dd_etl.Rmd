...
.extract_baselines <- function(dbf,tabname){
  if(F){
...
baseline_vals <- .extract_baselines(dbif,tabname="baseline_codes")
baseline_vals %>% group_by(baseline) %>% summarise(N=n(),.groups="drop") %>% display_data(disp=T)
```
